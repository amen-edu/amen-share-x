...
As amen Share X is in active development, see the release notes for breaking changes before upgrading.
...
1. Stop the running app
   ...
   ```bash
   pm2 stop amen-share-x-backend amen-share-x-frontend
   ```
...
2. Repeat the steps from the [installation guide](#stand-alone-installation) except the `git clone` step.
   ...
   npm install
   npm run build
   pm2 restart amen-share-x-backend

   # Start the frontend
   cd ../frontend
   npm install
   npm run build
   pm2 restart amen-share-x-frontend
   ```
