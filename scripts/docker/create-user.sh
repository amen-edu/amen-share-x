...
if ! getent group amen-share-group > /dev/null 2>&1; then
    addgroup -g "$PGID" amen-share-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u amen-share > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G amen-share-group amen-share > /dev/null 2>&1
    else
